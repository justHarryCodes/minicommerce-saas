...
import { cacheDelPattern, cacheDel, CacheKey } from '@/lib/redis'
...
export async function GET() {
  ...
  // Self-referencing: top-level and subs all in categories table
...
export async function POST(req: NextRequest) {
  ...
  // Ensure unique slug within store (parent_id can differ, but slug + store + parent must be unique per schema)
  const exists = await queryOne(
    'SELECT id FROM categories WHERE store_id = $1 AND slug = $2 AND parent_id IS NOT DISTINCT FROM $3',
    [store.id, slug, parentId ?? null]
  )
  ...
  `, [store.id, parentId ?? null, name, slug, sortOrder])
