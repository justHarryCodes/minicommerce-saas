...
import { Image, KeyboardAvoidingView, Modal, Platform, Pressable, ScrollView, StyleSheet, Switch, Text, View, ActivityIndicator } from 'react-native';
...
import { Camera, ChevronDown, FolderOpen, Trash2 } from 'lucide-react-native';
...
function snakeToCamel(obj: Record<string, unknown>): Record<string, unknown> {
  return Object.fromEntries(
    Object.entries(obj).map(([k, v]) => [
      k.replace(/_([a-z])/g, (_, c: string) => c.toUpperCase()), v,
    ])
  );
}
...
export default function EditProductScreen() {
  ...
  const { data: product, isLoading } = useQuery<Product>({
    queryKey: ['product', id],
    queryFn: async () => {
      const raw = await api.get<Record<string, unknown>>(`/api/products/${id}`);
      return snakeToCamel(raw) as unknown as Product;
    },
    enabled: !!id,
  });
  ...
  useEffect(() => {
    ...
    if (product.subcategoryId) setSubcategoryId(product.subcategoryId);
    if (product.categoryName) setCategoryLabel(product.categoryName);
    setReady(true);
  }, [product]);

  const update = useMutation({
    mutationFn: (body: Record<string, unknown>) => api.patch(`/api/products/${id}`, body),
    onSuccess: () => {
          ...
          <View style={styles.toggleCard}>
            {([
              ...
              <View key={key} style={[styles.toggleRow, i < arr.length - 1 && styles.toggleRowBorder]}>
                ...
                <Switch value={form[key]} onValueChange={(v) => setForm(f => ({ ...f, [key]: v }))} trackColor={{ true: Colors.brand }} thumbColor={Colors.white} />
              </View>
            ))}
          </View>

          <Button title="Save Changes" onPress={handleSubmit} loading={update.isPending} disabled={uploading} fullWidth size="lg" />
...
const styles = StyleSheet.create({
  ...
  toggleCard: {
    ...
    borderWidth: 1, borderColor: Colors.surface[200], overflow: 'hidden',
  },
  ...
  modalSheet: { backgroundColor: Colors.white, borderTopLeftRadius: 24, borderTopRightRadius: 24, maxHeight: '70%' },
  modalHeader: {
